...
export const useDashboardData = () => {
  ...
  const fetchClientsEvolution = useCallback(async () => {
    try {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      const alerts = generateAlerts(totalLeads, totalContratos, totalClientes);
  ...
  }, [fetchClientsEvolution, fetchRecentActivity]);


  const generateAlerts = (leads: number, contratos: number, clientes: number) => {
  ...
  const getTodayAppointmentsCount = (): number => {
    try {
      ...
      const events = JSON.parse(savedEvents) as Array<{ date: string; status: string }>;
